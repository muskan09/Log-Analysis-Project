...
ThirdQuery_Output["heading"]='The days on which more than 1% of requests lead to errors:- \n'
...
#prints the output to the query "What are the most popular three articles of all time?"
def print_article_query_results(query_output):
    print (query_output["heading"])
    for data in query_output["outputs"]:
        print ('\t\t\tArticle "' + str(data[0]) + '" has ' + str(data[1]) + ' views.')

#prints the output to the query "Who are the most popular article authors of all time?"
def print_author_query_results(query_output):
    print (query_output["heading"])
    for data in query_output["outputs"]:
        print ('\t\t\tAuthor "' + str(data[0]) + '" has ' + str(data[1]) + ' views.')

#prints the output to the query "On which days did more than 1% of requests lead to errors?"         
def print_error_query_results(query_output):
    print (query_output["heading"])
    for data in query_output["outputs"]:
        print ('\t\t\tThe day ' + str(data[0]) + ' had ' + str(data[1]) + '% errors.')
